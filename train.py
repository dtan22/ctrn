...
from model import CTRN
from data import QA_Dataset_CTRN
...
parser.add_argument(
    '--batch_size', default=250, type=int,
    help="Batch size."
)
...
parser.add_argument(
    '--save_to', default='m2', type=str,
    help="Where to save checkpoint."
)

parser.add_argument(
    '--mode', default='eval', type=str,
    help="Whether train or eval."
)
#--load_from
#s2
parser.add_argument(
    '--eval_split', default='test', type=str,
...
def append_QA_to_file(eval_log, answer,question,filename):
    f = open(filename, 'a+')
    for i,q in enumerate(question):
        f.write('%s     ' % q)
        f.write('%s  ' % answer[i])
        f.write('%s  ' % eval_log[i])
        f.write('\n')
    f.write('\n')
    f.close()
